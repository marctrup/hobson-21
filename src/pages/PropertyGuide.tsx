    ...
    <main>
      ...
      <div className="mx-auto grid max-w-6xl gap-12 px-6 py-14 lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <article className="min-w-0 max-w-3xl">
          ...
          {faqEntries.length > 0 && <section className="mb-12 border-t border-border pt-8" aria-labelledby="faq-heading"><h2 id="faq-heading" className="mb-5 font-serif text-3xl leading-tight">Frequently asked questions</h2><div className="divide-y divide-border border-y border-border">{guide.faqs?.map(faq => <details key={faq.q} className="group py-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-semibold marker:hidden">{faq.q}<span aria-hidden="true" className="shrink-0 text-muted-foreground transition-transform group-open:rotate-45">+</span></summary><p className="mt-3 text-base leading-8 text-muted-foreground">{faq.a}</p></details>)}</div></section>}
