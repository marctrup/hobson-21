    ...
    <div className="space-y-8 p-6">
      ...
      <div className="bg-gradient-to-r from-paper to-paper rounded-xl p-6">
        <h3 className="text-lg font-semibold text-ink mb-4 flex items-center gap-2">
          <Megaphone className="w-5 h-5 text-charcoal" />
          Top-Level Marketing Goals
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <p className="text-sm text-charcoal">Increase visibility across the UK real estate sector as the MVP moves into pilot expansion during 2026</p>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <p className="text-sm text-charcoal">Build credibility through evidence, pilot success stories, and trustworthy product education</p>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <p className="text-sm text-charcoal">Establish a predictable acquisition and activation funnel in preparation for the 2027 public launch</p>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
            <p className="text-sm text-charcoal">Support long-term adoption with consistent engagement and communication across all customer segments</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-bone p-6">
        ...
        <div className="flex items-stretch justify-between gap-4 overflow-x-auto pb-2">
          <div className="flex-1 min-w-[220px]">
            <div className="bg-warning-bg rounded-lg p-4 h-full">
              <p className="font-semibold text-warning mb-2">By Q4 2026</p>
              <p className="text-sm text-warning">Marketing should show strong early awareness, predictable interest in pilots, and growing engagement with educational content.</p>
            </div>
          </div>
          <ArrowRight className="w-6 h-6 text-ink-faint flex-shrink-0 self-center" />
          <div className="flex-1 min-w-[220px]">
            <div className="bg-bone-wash rounded-lg p-4 h-full">
              <p className="font-semibold text-charcoal mb-2">By Q4 2027</p>
              <p className="text-sm text-ink">Hobson should operate a scalable acquisition engine with mature digital channels, predictable lead flow, and clear conversion pathways following the public launch.</p>
            </div>
          </div>
          <ArrowRight className="w-6 h-6 text-ink-faint flex-shrink-0 self-center" />
          <div className="flex-1 min-w-[220px]">
            <div className="bg-bone-wash rounded-lg p-4 h-full">
              <p className="font-semibold text-charcoal mb-2">By 2028</p>
              <p className="text-sm text-ink">Marketing expands into two international regions and supports globalised demand, localisation, and partnerships.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {smartObjectives.map((stage, index) => {
            ...
            return (
              <div key={index} className="bg-white rounded-xl border border-bone overflow-hidden shadow-sm">
                ...
                <div className="p-4 space-y-4">
                  {/* Objectives */}
                  <div>
                    <p className="text-xs font-medium text-ink-muted uppercase mb-2">Objectives</p>
                    <ul className="space-y-2">
                      {stage.objectives.map((obj, objIndex) => (
                        <li key={objIndex} className="flex items-start gap-2 text-sm text-charcoal">
                  ...
                  <div className="bg-paper rounded-lg p-3">
                    <p className="text-xs font-medium text-ink-muted uppercase mb-1">Key Metrics</p>
                    <p className="text-sm text-charcoal">{stage.metrics}</p>
                  </div>

                  {/* Target Values */}
                  <div>
                    <p className="text-xs font-medium text-ink-muted uppercase mb-2">Target Values</p>
                    <div className="flex flex-wrap gap-2">
                      {stage.targets.map((target, targetIndex) => (
                        <div key={targetIndex} className="bg-paper border border-faint-rule rounded-lg px-3 py-2">
                          <span className="font-bold text-charcoal">{target.value}</span>
                          <span className="text-xs text-charcoal ml-1">{target.label}</span>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <div className="bg-gradient-to-r from-paper to-paper rounded-xl p-6">
        <h3 className="text-lg font-semibold text-ink mb-4">How This Objective Framework Supports the Strategy</h3>
      ...
      <div className="bg-white rounded-xl border border-bone p-6">
        <h3 className="text-lg font-semibold text-ink mb-2 flex items-center gap-2">
          <MousePointer className="w-5 h-5 text-charcoal" />
          Channel and Metric Justification
        </h3>
        ...
        <div className="bg-paper rounded-lg p-4 mb-6">
          <h4 className="font-medium text-ink mb-3">Top-Level Broad Goals Supported by Channel Strategy</h4>
          <ul className="space-y-2">
            {broadGoals.map((goal, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-charcoal">
                <CheckCircle className="w-4 h-4 text-ink-muted mt-0.5 flex-shrink-0" />
                <span>{goal}</span>
              </li>
            ))}
          </ul>
        ...
        {/* Channel Evolution */}
        <div className="mb-6">
          <h4 className="font-medium text-ink mb-3">Mid- to Long-Term Vision Supported by Channel Strategy</h4>
          <div className="space-y-3">
            <div className="border-l-4 border-warning-border pl-4">
              <p className="font-medium text-warning">2026 MVP Validation & Pilot Growth</p>
              <p className="text-sm text-charcoal">Channels focus on education, clarity, and trust-building. Content-led channels (LinkedIn, website, knowledge hub, email) support pilot acquisition and early engagement.</p>
            </div>
            <div className="border-l-4 border-bone-strong pl-4">
              <p className="font-medium text-charcoal">2026-2027 Commercial Readiness</p>
              <p className="text-sm text-charcoal">Channels shift toward a predictable digital acquisition funnel. SEO, targeted LinkedIn activity, retargeting flows, email nurture, and structured content work together to build demand.</p>
            </div>
            <div className="border-l-4 border-bone-strong pl-4">
              <p className="font-medium text-charcoal">2027 and Beyond – Scale</p>
              <p className="text-sm text-charcoal">Channels evolve to support broader market reach and international expansion using YouTube, partnerships, regionalised content, and paid acquisition.</p>
            </div>
          </div>
        </div>
        ...
        <div>
          <h4 className="font-medium text-ink mb-3">How Channels & Metrics Support Key Digital Strategy Areas</h4>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-bone-wash">
                  <th className="text-left px-4 py-2 font-medium text-charcoal">Digital Strategy Area</th>
                  <th className="text-left px-4 py-2 font-medium text-charcoal">Supporting Channels</th>
                  <th className="text-left px-4 py-2 font-medium text-charcoal">Supporting Metrics</th>
                  <th className="text-left px-4 py-2 font-medium text-charcoal">How It Benefits Hobson</th>
                </tr>
              </thead>
              <tbody>
                {channelMetricsTable.map((row, index) => (
                  <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-paper"}>
                    <td className="px-4 py-3 font-medium text-charcoal">{row.area}</td>
                    <td className="px-4 py-3 text-charcoal">{row.channels}</td>
                    <td className="px-4 py-3 text-charcoal">{row.metrics}</td>
                    <td className="px-4 py-3 text-charcoal">{row.benefit}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="bg-gradient-to-r from-bone-wash to-bone rounded-xl p-6">
        <h3 className="text-lg font-semibold mb-4 text-ink">Channel & Metric Alignment</h3>
        <p className="text-charcoal mb-4">Hobson's channel and metric choices are tightly aligned to:</p>
