...
const timelineData: PhaseSection[] = [
  {
    ...
    status: "completed",
    objectives: [
      "Validated the systemic failure of existing property management systems through direct operator discovery:",
    ],
    subItems: [
      ["Compliance risk", "Lease complexity", "Arrears management", "Maintenance chaos", "Portfolio fragmentation"],
      ["AI lease abstraction", "AI compliance orchestration", "AI financial intelligence"],
    ],
  },
  {
    phase: "Sep – Dec 2024",
    ...
    objectives: [
      ...
      "Four active pilot partners across operator sizes and system environments:",
    ],
    subItems: [
...
const productPillars = [
  { workflow: "Onboarding & Setup", capability: "AI document ingestion, lease abstraction, risk flags", icon: FileText },
  { workflow: "Compliance & Safety", capability: "Compliance rule engines, predictive failure", icon: Shield },
  { workflow: "Leasing & Occupier", capability: "Risk scoring, deal forecasting", icon: Users },
  { workflow: "Lease Management", capability: "Market intelligence, renewal modelling", icon: Building2 },
  { workflow: "Rent Collection & Finance", capability: "Arrears prediction, cashflow forecasting", icon: DollarSign },
  { workflow: "Maintenance & Asset", capability: "Predictive maintenance, cost optimisation", icon: Wrench },
  { workflow: "Inspections", capability: "Computer vision defect detection", icon: Eye },
  { workflow: "Legal & Risk", capability: "Litigation risk scoring", icon: Scale },
  { workflow: "Voids & Re-leasing", capability: "Void forecasting, incentive modelling", icon: Home },
  { workflow: "Portfolio Strategy", capability: "Scenario modelling, portfolio optimisation", icon: BarChart3 },
];
    ...
    <div className="w-full space-y-8">
      ...
      <div className="space-y-4">
        ...
        {timelineData.map((section, index) => (
          <div
            key={section.phase}
            className={`bg-background border border-border rounded-lg overflow-hidden border-l-4 ${getStatusColor(section.status, index)}`}
          >
            <div className="px-4 py-3 flex items-center justify-between">
              ...
              <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                ...
                section.status === "in-progress" ? "bg-amber-100 text-amber-700" :
            ...
            <div className="px-4 pb-4">
              <ul className="space-y-2">
                {section.objectives.map((objective, objIndex) => (
                  <li key={objIndex} className="text-sm text-foreground flex items-start gap-2">
                    <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-primary flex-shrink-0" />
                    <span>{objective}</span>
                  </li>
                ))}
              </ul>
              {section.subItems && section.subItems.map((group, groupIndex) => (
                <div key={groupIndex} className="ml-4 mt-2">
                  {groupIndex === 0 && section.phase === "May – Aug 2024" && (
                    <p className="text-xs text-muted-foreground mb-1">Confirmed market need for:</p>
                  )}
                  {groupIndex === 1 && (section.phase === "May – Aug 2024" || section.phase === "Sep – Dec 2024") && (
      ...
      <div className="bg-gradient-to-br from-primary/5 to-purple-50 rounded-xl p-6 border border-primary/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {productPillars.map((pillar, index) => (
            <div key={index} className="bg-white rounded-lg p-3 border border-gray-200 flex items-start gap-3">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {businessTimeline.map((phase, index) => (
      ...
      <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {commercialImpact.map((impact, index) => (
