...
const renderCapitalRaiseStrategy = (
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
  doc.setFont("helvetica", "bold");
  doc.text("FUNDING REQUIREMENT", margin + 8, yPosition + 12);
  doc.setFontSize(PDF_CONFIG.fontSize.stat);
  doc.setTextColor(...PDF_CONFIG.primaryColor);
  doc.text("GBP 5M", margin + 8, yPosition + 30);
  yPosition += 50;
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "bold");
  doc.text("Use of Funds", margin + 8, yPosition + 12);
  
  const fundItems = [
    "Secure category leadership with production-grade platform, QA, and security",
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
  doc.setFont("helvetica", "normal");
  let fundY = yPosition + 24;
  fundItems.forEach(item => {
    doc.text("- " + item, margin + 12, fundY);
    fundY += 8;
  });
  yPosition += 60;
...
const renderAssumptions = (
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "normal");
  const introText = doc.splitTextToSize("All projections are grounded in published government statistics, OECD benchmarks, and validated cost models. Our conservative approach uses midpoint estimates within evidence-based ranges.", maxWidth);
  doc.text(introText, margin, yPosition);
  yPosition += introText.length * 5 + 8;

  // Key Assumptions Matrix - Full Table
  doc.setFillColor(...PDF_CONFIG.primaryBgLight);
  doc.roundedRect(margin, yPosition, maxWidth, 85, 3, 3, "F");
  doc.setDrawColor(...PDF_CONFIG.primaryBorder);
  doc.setLineWidth(0.3);
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.cardTitle);
  doc.setFont("helvetica", "bold");
  doc.text("Key Assumptions Summary", margin + 8, yPosition + 10);

  // Table headers
  const colWidths = [55, 35, 75];
  let tableY = yPosition + 18;
  doc.setFillColor(...PDF_CONFIG.bgLight);
  doc.rect(margin + 4, tableY, maxWidth - 8, 8, "F");
  doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.text("Assumption Category", margin + 8, tableY + 5);
  doc.text("Value", margin + 8 + colWidths[0], tableY + 5);
  doc.text("Source / Basis", margin + 8 + colWidths[0] + colWidths[1], tableY + 5);
  tableY += 10;

  // Table rows
  doc.setFont("helvetica", "normal");
  data.keyAssumptions.forEach((item, i) => {
    const rowBg = i % 2 === 0 ? PDF_CONFIG.bgWhite : PDF_CONFIG.primaryBgLight;
    doc.setFillColor(...rowBg);
    doc.rect(margin + 4, tableY, maxWidth - 8, 7, "F");
    
    doc.setFontSize(PDF_CONFIG.fontSize.small);
    doc.setTextColor(...PDF_CONFIG.textDark);
    doc.text(item.category, margin + 8, tableY + 5);
    doc.setTextColor(...PDF_CONFIG.primaryColor);
    doc.setFont("helvetica", "bold");
    doc.text(item.value, margin + 8 + colWidths[0], tableY + 5);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...PDF_CONFIG.textGray);
    doc.text(item.source, margin + 8 + colWidths[0] + colWidths[1], tableY + 5);
    tableY += 7;
  });
  yPosition += 95;
  ...
  // UK Market Section
  doc.setFillColor(...PDF_CONFIG.bgLight);
  doc.roundedRect(margin, yPosition, maxWidth, 96, 3, 3, "F");
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "bold");
  doc.text("UK Real Estate Market", margin + 10, yPosition + 14);

  // Layout constants
  const ukTop = yPosition;
  const ukStatsX = margin + 10;
  const ukStatsW = 60;
  const ukStatsY = ukTop + 22;

  // Helper: draw a value + label, clipped to box width by reducing font size
  const drawStatBox = (
    x: number,
    y: number,
    w: number,
    ...
    // Value font size: start at statLarge and reduce if needed
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...PDF_CONFIG.primaryColor);
    let valueFont = PDF_CONFIG.fontSize.statLarge;
    doc.setFontSize(valueFont);
    while (doc.getTextWidth(value) > w - 10 && valueFont > PDF_CONFIG.fontSize.body) {
      valueFont -= 1;
      doc.setFontSize(valueFont);
    }
    ...
    doc.text(labelLines, x + 5, y + 18);
  };

  // Left: stacked stats
  drawStatBox(ukStatsX, ukStatsY, ukStatsW, 26, data.ukMarket.totalBusinesses, "Total UK Businesses");
  drawStatBox(ukStatsX, ukStatsY + 32, ukStatsW, 26, data.ukMarket.realEstateBusinesses, "Real Estate (4.2%)");

  // Right: Size Breakdown table
  const breakdownX = ukStatsX + ukStatsW + 12;
  const segmentW = 44;
  const shareX = breakdownX + segmentW + 8;
  const countX = shareX + 16 + 8;
  ...
  // headers
  doc.setFontSize(PDF_CONFIG.fontSize.small);
  doc.text("Segment", breakdownX, ukStatsY + 14);
  doc.text("Share", shareX, ukStatsY + 14);
  doc.text("Count", countX, ukStatsY + 14);
  ...
  let rowY = ukStatsY + 22;
  breakdownItems.forEach((item) => {
    const segLines = doc.splitTextToSize(item.size, segmentW);
    const rowH = Math.max(1, segLines.length) * PDF_CONFIG.lineHeight.tight;

    doc.setTextColor(...PDF_CONFIG.textGray);
    doc.text(segLines, breakdownX, rowY);
    doc.text(item.pct, shareX, rowY);

    doc.setTextColor(...PDF_CONFIG.primaryColor);
    doc.setFont("helvetica", "bold");
    doc.text(item.count, countX, rowY);
    doc.setFont("helvetica", "normal");
  ...
  doc.setFillColor(...PDF_CONFIG.bgLight);
  doc.roundedRect(margin, yPosition, maxWidth, 54, 3, 3, "F");
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "bold");
  doc.text("Global Market Scaling", margin + 10, yPosition + 14);

  const globalStatsY = yPosition + 22;
  const gap = 8;
  const globalBoxW = (maxWidth - 20 - gap * 3) / 4;
  const boxH = 22;

  const globalBoxes = [
    { value: data.globalScaling.oecdPopulation, label: "OECD Pop.", highlight: false },
    { value: data.globalScaling.ukPopulation, label: "UK Pop.", highlight: false },
    { value: data.globalScaling.multiplier, label: "Multiplier", highlight: false },
    { value: data.globalScaling.globalBusinesses, label: "Global RE", highlight: true },
  ];

  globalBoxes.forEach((b, idx) => {
    const x = margin + 10 + idx * (globalBoxW + gap);
    doc.setFillColor(...(b.highlight ? PDF_CONFIG.primaryBgMedium : PDF_CONFIG.primaryBgLight));
    doc.roundedRect(x, globalStatsY, globalBoxW, boxH, 2, 2, "F");

    doc.setFont("helvetica", "bold");
    doc.setTextColor(...(b.highlight ? PDF_CONFIG.primaryColor : PDF_CONFIG.textDark));

    let valueFont = PDF_CONFIG.fontSize.bodySmall;
    doc.setFontSize(valueFont);
    while (doc.getTextWidth(b.value) > globalBoxW - 10 && valueFont > PDF_CONFIG.fontSize.small) {
      valueFont -= 1;
      doc.setFontSize(valueFont);
    }
    doc.text(b.value, x + 5, globalStatsY + 10);

    doc.setFont("helvetica", "normal");
    doc.setTextColor(...PDF_CONFIG.textGray);
    doc.setFontSize(PDF_CONFIG.fontSize.small);
    doc.text(b.label, x + 5, globalStatsY + 18);
  });
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "bold");
  doc.text("Cost Structure Assumptions", margin + 8, yPosition + 12);
  
  // Three columns for cost structure
  const costColWidth = (maxWidth - 30) / 3;
  const costY = yPosition + 20;
  
  // Fixed Team
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.roundedRect(margin + 8, costY, costColWidth, 48, 2, 2, "F");
  doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.text("Fixed Team", margin + 12, costY + 10);
  doc.setTextColor(...PDF_CONFIG.primaryColor);
  doc.text(data.costStructure.fixedTeam, margin + 12, costY + 18);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...PDF_CONFIG.textGray);
  doc.setFontSize(PDF_CONFIG.fontSize.small);
  doc.text("CEO, Product, Marketing,", margin + 12, costY + 28);
  doc.text("Sales, Support leads", margin + 12, costY + 35);
  
  // Variable Costs
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.roundedRect(margin + 14 + costColWidth, costY, costColWidth, 48, 2, 2, "F");
  doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.text("Variable Costs", margin + 18 + costColWidth, costY + 10);
  doc.setTextColor(...PDF_CONFIG.primaryColor);
  doc.text(data.costStructure.variableCosts, margin + 18 + costColWidth, costY + 18);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...PDF_CONFIG.textGray);
  doc.setFontSize(PDF_CONFIG.fontSize.small);
  doc.text("Eng 12%, Marketing 8%,", margin + 18 + costColWidth, costY + 28);
  doc.text("CS 5%, Sales 4%, G&A 3%", margin + 18 + costColWidth, costY + 35);
  
  // AI & Infra
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.roundedRect(margin + 20 + costColWidth * 2, costY, costColWidth, 48, 2, 2, "F");
  doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.text("AI & Infra", margin + 24 + costColWidth * 2, costY + 10);
  doc.setTextColor(...PDF_CONFIG.primaryColor);
  doc.text(data.costStructure.infraCosts, margin + 24 + costColWidth * 2, costY + 18);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(...PDF_CONFIG.textGray);
  doc.setFontSize(PDF_CONFIG.fontSize.small);
  doc.text("Storage 4%, LLM 5%,", margin + 24 + costColWidth * 2, costY + 28);
  doc.text("K-Graph 2%, Monitor 1%", margin + 24 + costColWidth * 2, costY + 35);
  ...
  doc.setDrawColor(...PDF_CONFIG.emeraldBorder);
  doc.setLineWidth(0.3);
  doc.roundedRect(margin, yPosition, maxWidth, 40, 3, 3, "S");
  
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "bold");
  doc.text("AI Reading in Document Cost Per Client (One-Off)", margin + 8, yPosition + 12);
  
  const onboardingY = yPosition + 18;
  const onboardingColWidth = (maxWidth - 30) / 3;
  
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.roundedRect(margin + 8, onboardingY, onboardingColWidth, 16, 2, 2, "F");
  doc.roundedRect(margin + 14 + onboardingColWidth, onboardingY, onboardingColWidth, 16, 2, 2, "F");
  doc.roundedRect(margin + 20 + onboardingColWidth * 2, onboardingY, onboardingColWidth, 16, 2, 2, "F");
  
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setTextColor(...PDF_CONFIG.emerald);
  doc.setFont("helvetica", "bold");
  doc.text(data.costStructure.onboardingCosts.small, margin + 12, onboardingY + 10);
  doc.text(data.costStructure.onboardingCosts.medium, margin + 18 + onboardingColWidth, onboardingY + 10);
  doc.text(data.costStructure.onboardingCosts.large, margin + 24 + onboardingColWidth * 2, onboardingY + 10);
  
  yPosition += 50;

  // Check page break for Financial Policy
  yPosition = checkPageBreak(doc, yPosition, 70, pageHeight, margin);
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "bold");
  doc.text("Financial & Accounting Policy", margin + 8, yPosition + 12);
  
  const policyY = yPosition + 20;
  const policyCol = (maxWidth - 20) / 2;
  
  // Model Parameters
  doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
  doc.setFont("helvetica", "bold");
  doc.text("Model Parameters", margin + 8, policyY);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(PDF_CONFIG.fontSize.small);
  
  const modelParams = [
    ["Accounting Basis", data.financialPolicy.accountingBasis],
    ["Revenue Recognition", data.financialPolicy.revenueRecognition],
    ["Model Horizon", data.financialPolicy.modelHorizon],
    ["Corporation Tax", data.financialPolicy.corporationTaxRate],
  ];
  
  let paramY = policyY + 7;
  modelParams.forEach(([label, value]) => {
    doc.setTextColor(...PDF_CONFIG.textGray);
    doc.text(label + ":", margin + 8, paramY);
    doc.setTextColor(...PDF_CONFIG.textDark);
    doc.text(value, margin + 50, paramY);
    paramY += 5;
  });
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.text("Cost Classification", margin + 10 + policyCol, policyY);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(PDF_CONFIG.fontSize.small);
  
  const costClass = [
    ["Operating Costs", "Expensed"],
    ["R&D Treatment", "Expensed"],
    ["Infrastructure", "Included in COGS"],
    ["Model Currency", data.financialPolicy.modelCurrency],
  ];
  
  paramY = policyY + 7;
  costClass.forEach(([label, value]) => {
    doc.setTextColor(...PDF_CONFIG.textGray);
    doc.text(label + ":", margin + 10 + policyCol, paramY);
    doc.setTextColor(...PDF_CONFIG.textDark);
    doc.text(value, margin + 55 + policyCol, paramY);
    paramY += 5;
  });
...
/**
 * Render Financials Executive Summary visual
 */
const renderFinancialsExecutiveSummary = (
  ...
  let yPosition = startY;
  const maxWidth = pageWidth - margin * 2;
  const data = getFinancialsExecutiveSummaryStructuredData();
  ...
  const metrics = [
    ...
    { label: "ARR 2031", value: "GBP 37.4M" },
    { label: "Net Margin", value: "40-55%" }
  ];
  
  const boxWidth = (maxWidth - 20) / 3;
  metrics.forEach((metric, i) => {
    const xPos = margin + (boxWidth + 10) * i;
    doc.setFillColor(...PDF_CONFIG.primaryBgLight);
    doc.roundedRect(xPos, yPosition, boxWidth, 35, 3, 3, "F");
    doc.setTextColor(...PDF_CONFIG.textGray);
    doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
    doc.setFont("helvetica", "normal");
    doc.text(metric.label, xPos + 8, yPosition + 12);
    doc.setTextColor(...PDF_CONFIG.primaryColor);
    doc.setFontSize(PDF_CONFIG.fontSize.cardTitle);
    doc.setFont("helvetica", "bold");
    doc.text(metric.value, xPos + 8, yPosition + 26);
  });
  yPosition += 45;
...
/**
 * Render P/L Growth visual with bar chart representation and financial table
 */
const renderPLGrowth = (
  ...
  const plData = [
    { year: "2026", revenue: 0, cogs: 0.113, opex: 1.79, ebitda: -1.9, grossProfit: -0.113, grossMargin: "-" },
    { year: "2027", revenue: 0.708, cogs: 0.2, opex: 2.61, ebitda: -2.1, grossProfit: 0.508, grossMargin: "88%" },
    { year: "2028", revenue: 6.4, cogs: 0.66, opex: 5.08, ebitda: 0.69, grossProfit: 5.78, grossMargin: "92%" },
    { year: "2029", revenue: 22.4, cogs: 1.85, opex: 10.71, ebitda: 9.84, grossProfit: 20.55, grossMargin: "92%" },
    { year: "2030", revenue: 48.1, cogs: 3.9, opex: 21.08, ebitda: 23.1, grossProfit: 44.16, grossMargin: "92%" },
    { year: "2031", revenue: 99.7, cogs: 8.04, opex: 40.7, ebitda: 51.0, grossProfit: 91.66, grossMargin: "92%" },
  ];
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.sectionTitle);
  doc.setFont("helvetica", "bold");
  doc.text("P/L Growth Forecast", margin, yPosition);
  yPosition += 6;
  doc.setTextColor(...PDF_CONFIG.textGray);
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "normal");
  doc.text("2026-2031 Projected Financial Performance", margin, yPosition);
  yPosition += 12;
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
  doc.setFont("helvetica", "normal");
  const summaryText = doc.splitTextToSize("EBITDA breakeven in Year 3 (2028) and GBP 51M EBITDA by 2031. The model combines a lean fixed cost base (GBP 460k-680k/year) with AI-driven automation delivering 92% gross margins at scale.", maxWidth - 16);
  doc.text(summaryText, margin + 8, yPosition + 20);
  yPosition += 48;
  ...
  plData.forEach((d, i) => {
    ...
    if (d.ebitda >= 0) {
      ...
      doc.setFillColor(...PDF_CONFIG.redLight);
      doc.rect(barX + barWidth / 2, zeroLineY, barWidth / 2 - 1, ebitdaHeight, "F");
    }
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "bold");
  doc.text("Financial Summary (GBP)", margin + 8, yPosition + 10);
  
  // Table headers
  const tableY = yPosition + 16;
  const colWidths = [35, 22, 22, 22, 22, 22, 25];
  doc.setFillColor(...PDF_CONFIG.bgLight);
  doc.rect(margin + 4, tableY, maxWidth - 8, 8, "F");
  ...
  headers.forEach((h, i) => {
    doc.text(h, headerX, tableY + 5);
    headerX += colWidths[i];
  });
  
  // Table rows
  const rows = [
    { metric: "Total Revenue", values: ["GBP 0", "GBP 708k", "GBP 6.4M", "GBP 22.4M", "GBP 48.1M", "GBP 99.7M"], color: PDF_CONFIG.blue },
    { metric: "COGS", values: ["GBP 113k", "GBP 200k", "GBP 658k", "GBP 1.9M", "GBP 3.9M", "GBP 8.0M"], color: PDF_CONFIG.textGray },
    { metric: "Gross Profit", values: ["(GBP 113k)", "GBP 508k", "GBP 5.8M", "GBP 20.5M", "GBP 44.2M", "GBP 91.7M"], color: PDF_CONFIG.emerald },
    { metric: "Gross Margin", values: ["-", "88%", "92%", "92%", "92%", "92%"], color: PDF_CONFIG.textGray },
    { metric: "Total OpEx", values: ["GBP 1.8M", "GBP 2.6M", "GBP 5.1M", "GBP 10.7M", "GBP 21.1M", "GBP 40.7M"], color: PDF_CONFIG.textGray },
    { metric: "EBITDA", values: ["(GBP 1.9M)", "(GBP 2.1M)", "GBP 0.7M", "GBP 9.8M", "GBP 23.1M", "GBP 51.0M"], color: PDF_CONFIG.emerald },
  ];
  
  let rowY = tableY + 10;
  doc.setFont("helvetica", "normal");
  rows.forEach((row, ri) => {
    const rowBg = ri % 2 === 0 ? PDF_CONFIG.bgWhite : PDF_CONFIG.primaryBgLight;
    doc.setFillColor(...rowBg);
    doc.rect(margin + 4, rowY, maxWidth - 8, 8, "F");
    
    doc.setTextColor(...row.color);
    doc.setFont("helvetica", ri === 0 || ri === 2 || ri === 5 ? "bold" : "normal");
    doc.setFontSize(PDF_CONFIG.fontSize.small);
    
    let cellX = margin + 6;
    doc.setTextColor(...PDF_CONFIG.textDark);
    doc.text(row.metric, cellX, rowY + 5);
    cellX += colWidths[0];
    
    doc.setTextColor(...row.color);
    row.values.forEach((v, vi) => {
      doc.text(v, cellX, rowY + 5);
      cellX += colWidths[vi + 1];
    });
...
/**
 * Render Revenue Growth visual with chart and investor metrics
 */
const renderRevenueGrowth = (
  ...
  const revenueData = [
    { year: "2026", uk: 0, global: 0, total: 0, customers: 0 },
    { year: "2027", uk: 0.708, global: 0, total: 0.708, customers: 1168 },
    { year: "2028", uk: 1.624, global: 4.813, total: 6.436, customers: 10152 },
    { year: "2029", uk: 3.149, global: 19.250, total: 22.400, customers: 35705 },
    { year: "2030", uk: 5.591, global: 42.471, total: 48.061, customers: 82936 },
    { year: "2031", uk: 9.100, global: 90.596, total: 99.696, customers: 166302 },
  ];
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "normal");
  doc.text("2026-2031 Revenue Projections", margin, yPosition);
  yPosition += 12;
  ...
  doc.setFontSize(PDF_CONFIG.fontSize.bodySmall);
  doc.setFont("helvetica", "normal");
  const summaryText = doc.splitTextToSize("Projects GBP 99.7M ARR by 2031 with 166,000+ customers across UK and global markets. Anchored on conservative penetration rates (1.8% UK, 0.8% global) within a 4.5M operator addressable market.", maxWidth - 16);
  doc.text(summaryText, margin + 8, yPosition + 20);
  yPosition += 48;

  // Key Investor Metrics - 4 boxes
  const metricBoxWidth = (maxWidth - 18) / 4;
  const metrics = [
    { label: "2031 ARR", value: "GBP 99.7M", sublabel: "total revenue", bgColor: PDF_CONFIG.amberBg, borderColor: PDF_CONFIG.amberBorder, textColor: PDF_CONFIG.amber },
    { label: "Customers", value: "166k", sublabel: "by 2031", bgColor: PDF_CONFIG.blueBg, borderColor: PDF_CONFIG.blueBorder, textColor: PDF_CONFIG.blue },
    { label: "MRR 2031", value: "GBP 8.3M", sublabel: "monthly recurring", bgColor: PDF_CONFIG.emeraldBg, borderColor: PDF_CONFIG.emeraldBorder, textColor: PDF_CONFIG.emerald },
    { label: "Blended ARPU", value: "~GBP 600", sublabel: "per customer/year", bgColor: PDF_CONFIG.primaryBgLight, borderColor: PDF_CONFIG.primaryBorder, textColor: PDF_CONFIG.primaryColor },
  ];
  
  metrics.forEach((m, i) => {
    const boxX = margin + i * (metricBoxWidth + 6);
    doc.setFillColor(...m.bgColor);
    doc.roundedRect(boxX, yPosition, metricBoxWidth, 38, 3, 3, "F");
    doc.setDrawColor(...m.borderColor);
    doc.setLineWidth(0.3);
    doc.roundedRect(boxX, yPosition, metricBoxWidth, 38, 3, 3, "S");
    
    doc.setTextColor(...m.textColor);
    doc.setFontSize(PDF_CONFIG.fontSize.small);
    doc.setFont("helvetica", "bold");
    doc.text(m.label, boxX + 6, yPosition + 10);
    
    doc.setFontSize(PDF_CONFIG.fontSize.cardTitle);
    doc.text(m.value, boxX + 6, yPosition + 22);
    
    doc.setFontSize(PDF_CONFIG.fontSize.small);
    doc.setFont("helvetica", "normal");
    doc.text(m.sublabel, boxX + 6, yPosition + 32);
  });
  yPosition += 48;
  ...
  revenueData.forEach((d, i) => {
    ...
    // UK bar (amber)
    if (d.uk > 0) {
      const ukHeight = (d.uk / maxValue) * chartHeight;
      doc.setFillColor(...PDF_CONFIG.amber);
      doc.roundedRect(barX, chartY + chartHeight - ukHeight, barWidth, ukHeight, 1, 1, "F");
    }
    
    // Global bar stacked on top (blue)
    if (d.global > 0) {
      const ukHeight = (d.uk / maxValue) * chartHeight;
      const globalHeight = (d.global / maxValue) * chartHeight;
      doc.setFillColor(...PDF_CONFIG.blue);
      doc.roundedRect(barX, chartY + chartHeight - ukHeight - globalHeight, barWidth, globalHeight, 1, 1, "F");
    }
  ...
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.roundedRect(margin, yPosition, maxWidth, 72, 3, 3, "F");
  doc.setDrawColor(...PDF_CONFIG.primaryBorder);
  doc.setLineWidth(0.3);
  doc.roundedRect(margin, yPosition, maxWidth, 72, 3, 3, "S");
  
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.setFontSize(PDF_CONFIG.fontSize.body);
  doc.setFont("helvetica", "bold");
  doc.text("Investor Summary Matrix", margin + 8, yPosition + 10);
  ...
  // Table rows
  const rows = [
    { metric: "UK Revenue", values: ["GBP 0", "GBP 708k", "GBP 1.6M", "GBP 3.1M", "GBP 5.6M", "GBP 9.1M"], color: PDF_CONFIG.amber },
    { metric: "Global Revenue", values: ["GBP 0", "GBP 0", "GBP 4.8M", "GBP 19.3M", "GBP 42.5M", "GBP 90.6M"], color: PDF_CONFIG.blue },
    { metric: "Total Revenue", values: ["GBP 0", "GBP 708k", "GBP 6.4M", "GBP 22.4M", "GBP 48.1M", "GBP 99.7M"], color: PDF_CONFIG.primaryColor },
    { metric: "Customers", values: ["0", "1,168", "10,152", "35,705", "82,936", "166,302"], color: PDF_CONFIG.textGray },
    { metric: "Churn Rate", values: ["-", "5%", "7%", "8%", "8%", "8%"], color: PDF_CONFIG.textGray },
  ];
  ...
  rows.forEach((row, ri) => {
    ...
    doc.setFontSize(PDF_CONFIG.fontSize.small);
    doc.setFont("helvetica", ri === 2 ? "bold" : "normal");
...
/**
 * Render Go To Market Summary visual - based on SOSTAC framework
 * Uses PDF_CONFIG defaults for consistent spacing and no overlaps
 */
const renderGoToMarketSummary = (
  doc: jsPDF,
  startY: number,
  margin: number,
  pageWidth: number,
  ...
  let yPosition = startY;
  const maxWidth = pageWidth - margin * 2;
  const { box, spacing, fontSize, lineHeight } = PDF_CONFIG;
  ...
  // Header badge
  fitPage(25);
  doc.setFillColor(...PDF_CONFIG.tealBg);
  doc.roundedRect(margin, yPosition, 50, 14, box.borderRadius, box.borderRadius, "F");
  doc.setTextColor(...PDF_CONFIG.teal);
  doc.setFontSize(fontSize.bodySmall);
  doc.setFont("helvetica", "bold");
  doc.text("SOSTAC Framework", margin + 6, yPosition + 9);
  yPosition += 20;

  doc.setTextColor(...PDF_CONFIG.textGray);
  doc.setFontSize(fontSize.body);
  doc.setFont("helvetica", "normal");
  const subtitleLines = doc.splitTextToSize("Hobson — Investor Summary. A full SOSTAC-based Marketing Strategy document is available on request.", maxWidth);
  doc.text(subtitleLines, margin, yPosition);
  yPosition += subtitleLines.length * lineHeight.body + spacing.sectionGap;

  // ========== What We're Building ==========
  fitPage(75);
  const buildingBoxHeight = 70;
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.setDrawColor(...PDF_CONFIG.slateBorder);
  doc.setLineWidth(box.borderWidth);
  doc.roundedRect(margin, yPosition, maxWidth, buildingBoxHeight, box.borderRadius, box.borderRadius, "FD");
  
  // Icon
  doc.setFillColor(...PDF_CONFIG.blueBg);
  doc.roundedRect(margin + box.paddingX, yPosition + box.paddingX, 16, 16, 2, 2, "F");
  doc.setFillColor(...PDF_CONFIG.blue);
  doc.circle(margin + box.paddingX + 8, yPosition + box.paddingX + 8, 3, "F");
  
  // Title
  doc.setTextColor(...PDF_CONFIG.textDark);
  doc.setFontSize(fontSize.cardTitle);
  doc.setFont("helvetica", "bold");
  doc.text("What We're Building", margin + box.paddingX + 22, yPosition + box.paddingX + 11);
  
  // Content
  doc.setFontSize(fontSize.body);
  doc.setFont("helvetica", "normal");
  const buildingText = "Hobson is an AI assistant for real estate teams that turns existing documents into fast, accurate, referenced answers. Real estate organisations are overwhelmed by leases, agreements, and compliance documents. Finding simple information is slow, manual, and risky.";
  const buildingLines = doc.splitTextToSize(buildingText, maxWidth - box.paddingX * 2);
  doc.text(buildingLines, margin + box.paddingX, yPosition + 30);
  
  // Positioning badge
  doc.setFillColor(...PDF_CONFIG.blueBg);
  doc.roundedRect(margin + box.paddingX, yPosition + 52, maxWidth - box.paddingX * 2, 12, 2, 2, "F");
  doc.setTextColor(...PDF_CONFIG.blue);
  doc.setFontSize(fontSize.bodySmall);
  doc.setFont("helvetica", "bold");
  doc.text("Positioning: Disruption Without Displacement", margin + box.paddingX + 4, yPosition + 60);
  
  yPosition += buildingBoxHeight + spacing.cardGap;

  // ========== The Problem ==========
  fitPage(55);
  const problemBoxHeight = 50;
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.setDrawColor(...PDF_CONFIG.slateBorder);
  doc.roundedRect(margin, yPosition, maxWidth, problemBoxHeight, box.borderRadius, box.borderRadius, "FD");
  
  // Icon
  doc.setFillColor(...PDF_CONFIG.redBg);
  doc.roundedRect(margin + box.paddingX, yPosition + box.paddingX, 16, 16, 2, 2, "F");
  doc.setFillColor(...PDF_CONFIG.red);
  doc.circle(margin + box.paddingX + 8, yPosition + box.paddingX + 8, 3, "F");
  ...
  doc.setFontSize(fontSize.cardTitle);
  doc.setFont("helvetica", "bold");
  doc.text("The Problem", margin + box.paddingX + 22, yPosition + box.paddingX + 11);
  
  // Bullet points - 2 columns
  const problems = [
    "Critical information in unstructured documents",
    "Manual search wastes time, introduces errors",
    "Compliance and audit pressure increasing",
    "Teams leaner, expectations higher"
  ];
  
  doc.setFontSize(fontSize.bodySmall);
  doc.setFont("helvetica", "normal");
  const probColWidth = (maxWidth - box.paddingX * 2 - 10) / 2;
  problems.forEach((prob, i) => {
    const col = i % 2;
    const row = Math.floor(i / 2);
    const x = margin + box.paddingX + col * (probColWidth + 10);
    const y = yPosition + 30 + row * lineHeight.loose;
    doc.setFillColor(...PDF_CONFIG.red);
  ...
  yPosition += problemBoxHeight + spacing.cardGap;

  // ========== The Solution ==========
  fitPage(55);
  const solutionBoxHeight = 50;
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.setDrawColor(...PDF_CONFIG.slateBorder);
  doc.roundedRect(margin, yPosition, maxWidth, solutionBoxHeight, box.borderRadius, box.borderRadius, "FD");
  ...
  doc.setFontSize(fontSize.cardTitle);
  doc.setFont("helvetica", "bold");
  doc.text("The Solution", margin + box.paddingX + 22, yPosition + box.paddingX + 11);
  
  // Solution items - 2 columns
  const solutions = [
    { title: "Document-native:", desc: "Works from leases" },
    { title: "Referenced answers:", desc: "Shows sources" },
    { title: "No replacement:", desc: "Zero integration" },
    { title: "Low friction:", desc: "No onboarding" }
  ];
  
  const solColWidth = (maxWidth - box.paddingX * 2 - 10) / 2;
  solutions.forEach((sol, i) => {
    const col = i % 2;
    const row = Math.floor(i / 2);
    const x = margin + box.paddingX + col * (solColWidth + 10);
    const y = yPosition + 30 + row * lineHeight.loose;
    doc.setFontSize(fontSize.bodySmall);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...PDF_CONFIG.emerald);
    doc.text(sol.title, x, y);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(...PDF_CONFIG.textGray);
    doc.text(sol.desc, x + doc.getTextWidth(sol.title) + 2, y);
  });
  
  yPosition += solutionBoxHeight + spacing.cardGap;

  // ========== Who We Target ==========
  fitPage(60);
  const targetBoxHeight = 55;
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.setDrawColor(...PDF_CONFIG.slateBorder);
  doc.roundedRect(margin, yPosition, maxWidth, targetBoxHeight, box.borderRadius, box.borderRadius, "FD");
  ...
  doc.setFontSize(fontSize.cardTitle);
  doc.setFont("helvetica", "bold");
  doc.text("Who We Target (Phased)", margin + box.paddingX + 22, yPosition + box.paddingX + 11);
  
  // Phases as table rows
  const phases = [
    { phase: "Phase 1", title: "Large Portfolio Operators", label: "Primary" },
    { phase: "Phase 2", title: "Medium-Sized Property Companies", label: "" },
    { phase: "Phase 3", title: "Small Portfolio Owners", label: "Future" }
  ];
  
  let phaseY = yPosition + 30;
  phases.forEach((p) => {
    // Phase badge
    doc.setFillColor(...PDF_CONFIG.primaryBgLight);
    doc.roundedRect(margin + box.paddingX, phaseY - 3, 20, 7, 1, 1, "F");
    doc.setTextColor(...PDF_CONFIG.primaryColor);
    doc.setFontSize(fontSize.small);
    doc.setFont("helvetica", "bold");
    doc.text(p.phase, margin + box.paddingX + 2, phaseY + 2);
    
    // Title
    doc.setTextColor(...PDF_CONFIG.textDark);
    doc.setFontSize(fontSize.bodySmall);
    doc.setFont("helvetica", "bold");
    doc.text(p.title, margin + box.paddingX + 25, phaseY + 2);
    
    // Label if exists
    if (p.label) {
      doc.setTextColor(...PDF_CONFIG.textGray);
      doc.setFont("helvetica", "normal");
      doc.text("(" + p.label + ")", margin + box.paddingX + 25 + doc.getTextWidth(p.title) + 3, phaseY + 2);
    }
    
    phaseY += lineHeight.loose;
  });
  
  yPosition += targetBoxHeight + spacing.cardGap;

  // ========== Why We Win ==========
  fitPage(50);
  const winBoxHeight = 45;
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.setDrawColor(...PDF_CONFIG.slateBorder);
  doc.roundedRect(margin, yPosition, maxWidth, winBoxHeight, box.borderRadius, box.borderRadius, "FD");
  ...
  doc.setFontSize(fontSize.cardTitle);
  doc.setFont("helvetica", "bold");
  doc.text("Why We Win", margin + box.paddingX + 22, yPosition + box.paddingX + 11);
  
  // Win points - 2 columns
  const wins = [
  ...
  const winColWidth = (maxWidth - box.paddingX * 2 - 10) / 2;
  wins.forEach((win, i) => {
    const col = i % 2;
    const row = Math.floor(i / 2);
    const x = margin + box.paddingX + col * (winColWidth + 10);
    const y = yPosition + 30 + row * lineHeight.loose;
    doc.setFillColor(...PDF_CONFIG.amber);
    doc.circle(x + 2, y - 1, 1.2, "F");
    doc.setTextColor(...PDF_CONFIG.textDark);
  ...
  yPosition += winBoxHeight + spacing.cardGap;

  // ========== Timeline & Milestones ==========
  fitPage(55);
  const timelineBoxHeight = 50;
  doc.setFillColor(...PDF_CONFIG.bgWhite);
  doc.setDrawColor(...PDF_CONFIG.slateBorder);
  doc.roundedRect(margin, yPosition, maxWidth, timelineBoxHeight, box.borderRadius, box.borderRadius, "FD");
  ...
  doc.setFontSize(fontSize.cardTitle);
  doc.setFont("helvetica", "bold");
  doc.text("Timeline & Milestones", margin + box.paddingX + 22, yPosition + box.paddingX + 11);
  
  // Milestones as compact table
  const milestones = [
    { period: "2025-Q1 2026", items: "MVP completion, 5 pilot organisations" },
    { period: "2026", items: "3-5 paid customers, case studies, early revenue" },
    { period: "2027", items: "UK commercial launch, scalable onboarding" },
    { period: "2028+", items: "European/global expansion (US/APAC)" }
  ];
  
  let msY = yPosition + 30;
  milestones.forEach((ms) => {
    doc.setTextColor(...PDF_CONFIG.blue);
    doc.setFontSize(fontSize.small);
    doc.setFont("helvetica", "bold");
    doc.text(ms.period, margin + box.paddingX, msY);
    doc.setTextColor(...PDF_CONFIG.textGray);
    doc.setFontSize(fontSize.bodySmall);
    doc.setFont("helvetica", "normal");
    doc.text(ms.items, margin + box.paddingX + 35, msY);
    msY += lineHeight.body;
  });
...
const renderTabContent = (
  ...
  if (hasCustomVisual) {
    ...
    else if (componentType === "goToMarketSummary") {
      yPosition = renderGoToMarketSummary(doc, yPosition, margin, pageWidth, pageHeight);
